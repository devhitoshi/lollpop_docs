...
- **応答言語**: 常に日本語。
- **ブランチ**: 新規スレッド＝1タスク＝1ブランチ。`main` から `<video|article|review>/<YYYYMMDD>-<内容スラッグ>` を切り、
  そのスレッド内の変更はすべて同じブランチに積む。**スレッドの最初の応答でブランチ名を明示する**
  （セッションが切れても、その1行があれば再開できる）。完了指示を受けたら main にマージする。
