...
- **X収集は `.claude/skills/x-account-fetch`（twitterapi.io）に一本化した（2026-09-01〜）。** 公式・メンバーの投稿取得（手順1）とエゴサーチ・周囲の反応の取得（手順2）の両方に対応。手順2は取得はスクリプト、採用可否のノイズ判定は人間かLLMが別途行う。取得済みの生データは `x_cache/`（Git管理外の永続ローカルキャッシュ、詳細は同ディレクトリのREADMEとSKILL.md）に溜まるので、同じ期間・クエリなら再取得は不要。APIキーやネットワークが使えない環境ではGrok（`prompts/collect/x_collect.md`）にフォールバックする。

## 実行環境の注意（Claude Code リモート環境）

- **twitterapi.io は環境によって到達可否が変わる。** 信頼モードでは403、フルアクセス環境では到達可能（2026-09-01確認）。APIキーは環境変数 `TWITTERAPI_IO_KEY` かルートの `.env`（`.gitignore` 済み）から読む。**キーをチャットに貼らせない。** 取得した投稿データ（`work/x_fetch/`、`x_cache/`）は他人の著作物なのでコミットしない（`.gitignore` 済み）。
